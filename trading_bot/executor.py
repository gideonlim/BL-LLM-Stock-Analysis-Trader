...
from trading_bot.history import (
    TradeHistory,
...
from trading_bot.portfolio_optimizer import (
    rank_intents_by_marginal_sharpe,
)
...
def execute(
    ...
    #    Rank BUY intents by marginal contribution to portfolio
    #    Sharpe ratio rather than just raw composite score.
    buy_intents = [i for i in intents if i.side == "buy"]
    sell_intents = [i for i in intents if i.side != "buy"]

    if len(buy_intents) >= 2:
        ranked = rank_intents_by_marginal_sharpe(
            buy_intents,
            held_positions=portfolio.positions,
            portfolio_equity=portfolio.equity,
        )
        # Reorder intents: sells first, then buys by marginal Sharpe
        intents = sell_intents + [r.intent for r in ranked]
    else:
        log.info(
            "  Skipping portfolio optimization "
            f"({len(buy_intents)} BUY intent — need >= 2)"
        )
    ...
    for intent in intents:
        ...
        if intent.side == "sell" and intent.signal.signal_raw == -1:
            ...
            # Bracket orders require whole shares, so we pass
            # current_price for the qty calculation
            result = broker.submit_bracket_order(
                ...
                current_price=intent.signal.current_price,
